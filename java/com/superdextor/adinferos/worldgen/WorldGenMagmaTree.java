...
public class WorldGenMagmaTree extends WorldGenAbstractTree implements IOverridable
{
    private static final IBlockState log = NetherBlocks.LOG.getDefaultState().withProperty(BlockNetherLog.VARIANT, BlockNetherPlanks.EnumType.MAGMA);
    private static final IBlockState leaves = NetherBlocks.LEAVES.getDefaultState().withProperty(BlockNetherLeaf.VARIANT, BlockNetherPlanks.EnumType.MAGMA).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (this.doOverride || k >= 1 && k + i + 1 < 256)
        {
            ...
            if(!this.doOverride && this.playerMade) {
                if (state.getBlock() != Blocks.LAVA && block != Blocks.NETHERRACK && block != Blocks.SOUL_SAND && block != NetherBlocks.DARKSTONE && block != NetherBlocks.DARK_SAND && block != NetherBlocks.GOLD_ORE && block != NetherBlocks.DIMENSIONAL_ORE && block != NetherBlocks.NETHERITE_ORE && block != NetherBlocks.WITHER_ORE && block != Blocks.QUARTZ_ORE)
                {
                    return false;
                }
            }
            ...
            else if (!this.doOverride && !this.func_181638_a(worldIn, position, i))
            {
                return false;
            }
            else
    ...
    private boolean func_181638_a(World p_181638_1_, BlockPos p_181638_2_, int p_181638_3_)
    {
        ...
        for (int l = 0; l <= p_181638_3_ + 1; ++l)
        {
            ...
            for (int j1 = -i1; j1 <= i1; ++j1)
            {
                ...
                {
                    if (!this.isReplaceable(p_181638_1_, blockpos$mutableblockpos.setPos(i + j1, j + l, k + k1)))
                    {
                        return false;
                    }
                }
    ...
    private void func_181639_b(World p_181639_1_, BlockPos p_181639_2_)
    {
        ...
        {
            this.setBlockAndNotifyAdequately(p_181639_1_, p_181639_2_, log);
        }
    }

...
public class WorldGenInfernoTree extends WorldGenAbstractTree implements IOverridable
{
    private Random rand;
    private World world;
    ...
    List<WorldGenInfernoTree.FoliageCoordinates> field_175948_j;
    
    public boolean doOverride = false;

    public WorldGenInfernoTree(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.field_175948_j = Lists.<WorldGenInfernoTree.FoliageCoordinates>newArrayList();
        this.field_175948_j.add(new WorldGenInfernoTree.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.field_175948_j.add(new WorldGenInfernoTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.func_181631_a(pos.up(i), this.leafSize(i), NetherBlocks.LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenInfernoTree.FoliageCoordinates WorldGenInfernoTree$foliagecoordinates : this.field_175948_j)
        {
            this.generateLeafNode(WorldGenInfernoTree$foliagecoordinates);
        }
    }
    ...
    void generateLeafNodeBases()
    {
        for (WorldGenInfernoTree.FoliageCoordinates WorldGenInfernoTree$foliagecoordinates : this.field_175948_j)
        {
            int i = WorldGenInfernoTree$foliagecoordinates.func_177999_q();
            BlockPos blockpos = new BlockPos(this.basePos.getX(), i, this.basePos.getZ());

            if (!blockpos.equals(WorldGenInfernoTree$foliagecoordinates) && this.leafNodeNeedsBase(i - this.basePos.getY()))
            {
                this.func_175937_a(blockpos, WorldGenInfernoTree$foliagecoordinates, NetherBlocks.LOG);
            }
        }
    }
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (!this.doOverride && !this.validTreeLocation())
        {
    ...
    private boolean validTreeLocation()
    {
        ...
        if (block != Blocks.NETHERRACK && block != Blocks.SOUL_SAND && block != NetherBlocks.DARKSTONE && block != NetherBlocks.DARK_SAND && block != NetherBlocks.GOLD_ORE && block != NetherBlocks.DIMENSIONAL_ORE && block != NetherBlocks.NETHERITE_ORE && block != NetherBlocks.WITHER_ORE && block != Blocks.QUARTZ_ORE)
        {
            return false;
        }
        else

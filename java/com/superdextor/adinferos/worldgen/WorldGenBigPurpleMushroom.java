...
public class WorldGenBigPurpleMushroom extends WorldGenerator implements IOverridable
{
	private int mushroomType = -1;
	public boolean doOverride = false;

    public WorldGenBigPurpleMushroom(int type)
    {
        super(true);
        this.mushroomType = type;
    }

    public WorldGenBigPurpleMushroom()
    {
        super(false);
    }
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (this.doOverride || position.getY() >= 1 && position.getY() + i + 1 < 256)
        {
            ...
            else
            {
                ...
                if (!this.doOverride && block1 != Blocks.NETHERRACK && block1 != Blocks.SOUL_SAND && block1 != NetherBlocks.GOLD_ORE && block1 != NetherBlocks.WITHER_ORE && block1 != NetherBlocks.NETHERITE_ORE && block1 != NetherBlocks.DIMENSIONAL_ORE && block1 != NetherBlocks.DARKSTONE && block1 != NetherBlocks.DARK_SAND)
                {
                    return false;
                }
                else
                {
                        ...
                        for (int l1 = k3; l1 <= l3; ++l1)
                        {
                            for (int i2 = j1; i2 <= k1; ++i2)
                            {
                                ...
                                if (position.getY() >= position.getY() + i - 1 || blockhugemushroom$enumtype != BlockHugeMushroom.EnumType.ALL_INSIDE)
                                {
                                    ...
                                    {
                                        this.setBlockAndNotifyAdequately(worldIn, blockpos, NetherBlocks.PURPLE_MUSHROOM_BLOCK.getDefaultState().withProperty(BlockHugeMushroom.VARIANT, blockhugemushroom$enumtype));
                                    }
                                }
                            }
                        }
                    ...
                    for (int i3 = 0; i3 < i; ++i3)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(worldIn, position.up(i3), NetherBlocks.PURPLE_MUSHROOM_BLOCK.getDefaultState().withProperty(BlockHugeMushroom.VARIANT, BlockHugeMushroom.EnumType.STEM));
                        }
                    }
